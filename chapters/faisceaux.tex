\chapter{Faisceaux divisoriels sur une variété algébrique normale}
...
\end{center}
En effet, tout élément homogène $f\in A_d$ définit une section globale de $\Oo_X(d)$ pour $d\in \ZZ$, puis pour tout $t\in \Gamma(X,\Ff(n)),\, n\in\ZZ$, on pose $ft=s\otimes_{\Oo(X)} t\in \Gamma(X, \Ff(n+d))$. Comme $A$ est engendrée par des éléments homogènes de degré $1$, on a un isomorphisme naturel 
\begin{center}
...
Réciproquement, considérons le $\mathcal{O}_X$-module des sections locales d'un fibré en droites $L$ sur $X$. Sur les ouverts $U_i$ où $L$ est trivialisé on voit que les sections locales forment un faisceau isomorphe à $\mathcal{O}_{U_i}$. En effet se donner une section sur $U_i$ revient à se donner un morphisme $U_i\rightarrow \AAA^1$, c'est-à-dire un élément de $\mathcal{O}_X(U_i)$. C'est donc un faisceau inversible. Les sections globales de $L$ s'identifie via les trivialisations locales aux familles $(f_i)_{i\in I}$ telles que $f_i=a_{ij}f_j$ sur $U_i\cap U_j$. En effet, un telle section donne sur les intersections $U_i\cap U_j$ un diagramme commutatif:
	...
	\begin{center}
	\begin{tikzcd}
  		U_i\cap U_j\times \AAA^1 \arrow[rr, "\id\times m_{a_{ij}}"] & & U_i\cap U_j\times \AAA^1 \\ 
  		& U_i\cap U_j \arrow[ul, "\id\times f_i"] \arrow[ru, swap, "\id\times f_j"] &
	\end{tikzcd}
...
Une action $G$-linéarisée définit donc un morphisme $G\rightarrow \Aut L$ à valeurs dans le groupe d'automorphismes de fibrés en droites de $(L,\pi)$. On note $\alpha: G\times X\rightarrow X$ l'action de $G$ sur $X$ et $p_2:G\times X\rightarrow X$ la projection sur $X$. Pour tout $g\in G$ on a une application:
