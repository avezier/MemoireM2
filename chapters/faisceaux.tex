...
\begin{enumerate}
\item Soit un diviseur de Weil $D:=\sum_i a_iD_i$, on a $\Gamma(X,\Oo_X(D))=\bigcap_{\haut \mathfrak{p}_i=1}(\mathfrak{p}_iA_{\mathfrak{p}_i})^{-a_i}$.
\item Pour un diviseur premier $D$ correspondant à un idéal premier $\mathfrak{p}$ de hauteur $1$, on a $\Gamma(X,\Oo_X(-D))=\mathfrak{p}$. Si de plus $\mathfrak{p}$ est inversible et $a\in \ZZ$, on a $\Gamma(X,\Oo_X(aD))=\mathfrak{p}^{-a}$.
\end{enumerate}
\end{prop}
\begin{proof}
\begin{enumerate}
...
Soit $X$ une variété irréductible. Un diviseur de Cartier sur $X$ est une section globale du faisceau $k(X)^*/\mathcal{O}_X^*$. Ainsi un diviseur de Cartier est la donnée d'une famille $(U_i, f_i)_{i\in I}$ telle que pour tout $i$, $U_i$ est un ouvert de $X$, $(U_i)_{i\in I}$ est un recouvrement de $X$, $f_i\in k(X)^*$, et pour tout $i,j \in I$, $f_if_j^{-1}\in \mathcal{O}_X^*(U_i\cap U_j)$.
...
\text{inversible et d'une section globale non-nulle}
\end{matrix}
\right\}
$$
...
On peut étendre la correspondance précédente aux diviseurs de Weil effectifs. En effet $\wdiv(X)\simeq\wdiv(X_{reg})$ et tout les diviseurs sont de Cartier sur $X_{reg}$. Partant d'un diviseur effectif $D\in \wdiv(X)$,
