...
Soit $X$ une variété normale et irréductible et $D\in\wdiv(X)$. Alors pour tous ouverts $V\subset U\subset X$ tels que $U\setminus V$ soit de codimension $\geq 2$ dans $U$, on a $\Oo_X(D)_{|V}\simeq \Oo_X(D)_{|U}$.
\end{prop}
\begin{proof}
\end{proof}
...
\end{cor}
\begin{proof}
Si $D$ est localement principal c'est immédiat car alors le module est inversible. Mais on peut toujours le supposer d'après la proposition précédente et \ref{codimesingnormal}.
\end{proof}
