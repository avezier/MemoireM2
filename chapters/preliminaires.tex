...
Dans ce mémoire, on travaille dans la catégorie des $k$-schémas réduits séparés de type fini sur $k$, où $k$ est un corps algébriquement clos de caractéristique zéro fixé. Ces objets sont appelés des $k$-variétés algèbriques, ou tout simplement variétés.  Cette catégorie est équivalente à la catégorie des $k$-variétés algébriques au sens de \cite{LAGSpringer}. Cela nous permettra d'utiliser l'un ou l'autre langage suivant la commodité.
On note que la sous-catégorie pleine des variétés affines est anti-équivalente à celle des $k$-algèbre de type fini réduites via le foncteur sections globales, noté $k[.]$ dans ce cas pour coller aux notations traditionnelles.
\\
Dans ce contexte toute partie localement fermée d'une variété admet une unique structure de variété.\\
Tout fermé se décompose de manière unique en une union finie de sous-variétés fermées irréductibles maximales.\\
Le produit sur $k$ existe et préserve l'irréductibilité.
...
\subsection{Diviseurs de Cartier, faisceaux inversibles, fibrés en droites}
...
Un diviseur de cartier est dit principal si il provient d'une section globale de $k(X)^\times$ c'est à dire d'un fonction rationnelle. Deux diviseurs de Cartier sont dits linéairement équivalents si ils sont égaux modulo le sous-groupe des diviseurs principaux. Le groupe quotient se note CaCl$(X)$.
\end{defn}
...
\subsection{Faisceaux amples, très amples. Caractérisation des variétés projectives.}
