...
\end{prop}
\begin{proof}
Par irréductibilité de $X$, cette flèche est injective. Pour la surjectivité on peut traiter le problème localement et supposer $X=\spec A$ affine. On considère $f$ régulière sur $X\setminus Y$, alors $f\in A_p$ pour tout $p\in \spec A$ de hauteur $1$. En effet, $p\notin Y$ car dans le cas contraire il contiendrait les idéaux premiers correspondants aux composantes irréductibles de $X\setminus Y$, ce qui est impossible car ils sont de hauteur $\geq 2$. On en déduit que $f\in A=\Oo(X)$ par application de \ref{factonormal}.
\end{proof}
...
\textcolor{red}{A ajouter?:}\\
\textcolor{red}{ Séparé => Cf Stacks 8.8 construction of schemes}\\
\textcolor{red}{irréductible = Cf critére avec recovrement}\\
\textcolor{red}{sections globales (cf hartshorne chap 1 si A0=k et A engendré par A1)? c'est propre?}\\
\textcolor{red}{Fonctorialité de Proj}\\
...
Ainsi l'application $k(X)^*\rightarrow $ WDiv$(X),\, f\mapsto $ div$(f):=\sum_D v_D(f)D$ définit un morphisme de groupe. Son image est le \textit{groupe des diviseurs principaux} noté PDiv$(X)$. La relation modulo PDiv$(X)$ s'appelle \textit{l'équivalence linéaire}, et le groupe quotient Cl$(X)$ est le \textit{groupe des classes de diviseurs}.\\
...
\begin{enumerate}
\item Soit un diviseur de Weil $D=a_{p_1}Y_{p_1}+...+a_{p_r}Y_{p_r}$, on a $\Gamma(X,\Oo_X(D))=\bigcap_{ht(p)=1}p^{-a_p}A_p$.
...
\end{prop}
\begin{proof}
Si $D$ est localement principal c'est immédiat car alors le module est inversible. Mais on peut toujours le supposer, voir \ref{preimagecodim2} qui s'applique dans ce cas et \ref{extregularnormal}.
\end{proof}
...
Le résultat suivant est un outil pratique pour calculer les sections d'un faisceau d'algèbres divisorielles.

\begin{prop}
Soit $X$ une variété normale irréductible, $D,E\in\wdiv X$ avec $E$ de Cartier. Alors le morphisme naturel $\Oo_X(D)\otimes_{\Oo_X}\Oo_X(E)\rightarrow \Oo_X(D+E)$ est un isomorphisme.
\end{prop}
\begin{proof}
Il suffit d'effectuer le calcul localement sur des ouverts qui trivialisent $\Oo_X(D+E)$. On obtient ainsi des isomorphismes qui se recollent.
\end{proof}

Par analogie avec les diviseurs de Weil, un diviseur de Cartier $D=(U_i, f_i)_{i\in I}$ est dit effectif si pour tout $i$, $f_i\in \Oo_X(U_i)$. Dans ce cas $\Oo_X(-D)$ est un sous $\Oo_X$-module de $\Oo_X$, c'est concrètement le faisceau d'idéaux localement généré sur chaque $U_i$ par $f_i$. D'après \ref{dimsousvariete} cela définit un sous-schéma fermé de $X$ de codimension 1. L'inclusion $\Oo_X(-D)\xhookrightarrow{} \Oo_X$ est une section globale de $\Hh om(\Oo_X(-D), \Oo_X)\simeq \Oo_X(D)$ appelée section canonique et notée $1_D$ puisqu'elle correspond à la multiplication par $1$. Réciproquement, la donnée d'un couple $(\Ll, s)$ constitué d'un faisceau inversible sur $X$ et d'une section globale définit un diviseur de Cartier effectif de la manière suivante. Soit $(U_i)_i$ un recouvrement qui trivialise $\Ll$. Sur chaque $U_i$ on a un isomorphisme $\phi_i:\Ll_{|U_i}\rightarrow\Oo_{X|U_i}$. On voit que $(U_i,\phi_i(s))_i$ définit un diviseur de Cartier effectif indépendant du choix des $\phi_i$, et donc de $\Ll$ à isomorphisme près. On l'appelle le diviseur des zéros de $s$ et on le note $\divi_D(s)$. Les deux procédés que l'on vient de décrire sont inverses l'un de l'autre, on obtient ainsi une correspondance bijective:

$$
\left\{
\begin{matrix}
\text{Diviseur de Cartier effectif sur }X
\end{matrix}
\right\}
\leftrightarrow
\left\{
...
Considérons un faisceau inversible $\Ll$ sur une variété $Y$, une section globale $l$, et un morphisme $(g,g^\sharp):X\rightarrow Y$. On définit l'ouvert $$Y_l:=\lbrace y\in Y\mid \Oo_{Y,y}l_y=\Ll_y\rbrace=\lbrace y\in Y\mid l_y\notin m_y\Ll_y\rbrace$$
Si $\Ll=\Oo_Y$ c'est tout simplement l'ouvert principal $Y_l$, et on a de plus $g^{-1}(Y_l)=X_{g^\sharp(l)}=X_{g^*(l)}$ car $g^*\Oo_Y=\Oo_X$ et $\alpha=g^\sharp$ dans ce cas. Revenant dans le cas général, on voit facilement que $g^*\Ll$ est inversible et en raisonnant localement sur des ouverts affines qui trivialisent $\Ll$, on voit d'après ce qui précède que $g^{-1}(Y_l)=X_{g^*(l)}$. D'autre part, si $l_1,...,l_n$ sont des sections globales qui engendrent $\Ll$, on voit que $(Y_{l_i})$ est un recouvrement de $Y$ qui trivialise $\Ll$. En effet, sur chaque $Y_{l_i}$, on a un isomorphisme $\Oo_{Y|Y_l}\rightarrow \Ll_{Y|Y_l}$, $\lambda \mapsto \lambda l_i$, et par hypothèse, l'intersection des complémentaires des $Y_{l_i}$ est vide. Enfin, on note sans ambiguïté $l_j/l_i$ l'unique élément de $\Oo_{Y|Y_l}(Y_l)$ tel que $l_j/l_i.l_i=l_j$ par l'isomorphisme précédent.\\
...
Notons que cette propriété caractérise l'espace projectif sur $k$ à isomorphisme près. En effet il représente le foncteur (voir stacks)...
...
Supposons maintenant $X$ lisse (donc normale) et projective, $\Ll\simeq \Oo_X(D), D\in \wdiv(X)$ un faisceau inversible, et $s$ une section globale non nulle. Par définition, $\divi_D(s)$ est un diviseur effectif linéairement équivalent à $D$.
Réciproquement on voit que tout diviseur effectif linéairement équivalent à $D$ est de la forme $\divi_D(s)+D$ pour une section globale $s$ de $\Oo_X(D)$. D'autre part deux sections globales $s, s'$ non nulles ont même diviseur des zéros si et seulement si elles sont colinéaires dans le $k$-ev $\Gamma(X, \Oo_X(D))$, en effet dans ce cas $s/s'\in \Oo_X^\times(X)=k^*$, car $k$ est algébriquement clos. Notons que $\Gamma(X, \Oo_X(D))$ est de dimension finie (\cite{Hartshorne} II.5.19), ainsi l'ensemble des diviseurs effectifs linéairement équivalent à $D$ est naturellement muni d'une structure d'espace projectif, cela amène la définition suivante:
