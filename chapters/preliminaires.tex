...
\begin{defn}
Un anneau local noethérien $A$ de corps résiduel $k=A/\mathfrak{m}$ est régulier si $\dim A =\mathfrak{m}/\mathfrak{m}^2$.
\end{defn}
...
Notre principale référence est \cite{Hartshorne}. On résume dans cette partie certains faits et définitions, souvent sans démonstration. Soit $A$ un anneau, $\mathfrak{p}$ un idéal premier de $A$, et $f\in A$ un élément. On note $A_\mathfrak{p}$ la localisation de $A$ en la partie multiplicative $A\setminus \mathfrak{p}$, et $A_f$ la localisation de $A$ en la partie multiplicative $\lbrace 1, f, f^2, ....\rbrace$.
...
Comme tout anneau est naturellement une $ZZ$-algèbre, tout schéma est naturellement un schéma sur $\spec \ZZ$ est le produit de deux schéma est par définition le produit sur $\spec \ZZ$.
...
\begin{prop}
Un schéma $X$ un schéma intègre et $k(X)$ son corps des fonctions rationnelles. 
\begin{enumerate}
...
\item Pour tout ouvert non-vide $U\subset X$ et tout recouvrement $U=\cup_i U_i$, on a 
...
\subsubsection{Immersions ouvertes, immersions fermées}
...
\begin{defn}
Un schéma est dit normal si tous ses anneaux locaux intégralement clos.
\end{defn}
...
Un point $x$ d'une variété $X$ irréductible est régulier si l'espace tangent $\mathfrak{m}_x/\mathfrak{m}_x^2$ en $x$ est de dimension $\dim X$. L'ensemble des point singulier d'une variété est un fermé propre.
...
\subsubsection{Dimension des sous-variétés}
