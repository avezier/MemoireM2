...
Les ensembles algébriques affines sont de la forme $\mathcal{V}(I):=\lbrace x\in k^n|\forall P\in I, P(x)=0\rbrace$ où $I$ est un idéal de $S$. Ils sont stables par intersection quelconque, et on a $\mathcal{V}(S)=\emptyset$ et $\mathcal{V}(\lbrace 0\rbrace)=k^n$. Ainsi les ensembles algébriques affines constituent les fermés d'une topologie, dite de Zariski.\\
Par ailleurs, à tout ensemble algébrique affine $\Sigma$ on associe l'idéal $\mathcal{I}(\Sigma):=\lbrace P\in k[X_1,...X_n] | P(x)=0, \forall x\in\Sigma \rbrace$. L'ensemble des morphismes $\Sigma\rightarrow k$ est doté d'une structure de k-algèbre évidente que l'on note $k[\Sigma]$. Cette algèbre est naturellement isomorphe à $S/\mathcal{I}(\Sigma)$. Elle est de type fini réduite, on dit que c'est une $k$-algèbre affine. D'après le Nullstellensatz on a $\mathcal{I}(\mathcal{V}(I))= \sqrt{I}$, d'où une bijection entre les sous-ensembles algébriques affines de $\Sigma$ et les idéaux radicaux de $k[\Sigma]$. En particulier les points de $\Sigma$ sont en bijection avec les idéaux maximaux de $k[\Sigma]$. On note $Specm(k[\Sigma])$ cet ensemble.
...
\begin{defn}
Une représentation de $G$, ou $G$-module est un couple $(V, \rho)$ ou
semi-simple module.
\end{defn}
...
\begin{thm}
Soit $G$ un groupe algébrique, $H\leq G$ fermé. \\Alors il existe un $G$-module $V$ de dimension finie et une ligne $L\subset V$ telle que $H=Stab_G(L):=\lbrace g\in G| g.v\in L,\forall v\in L\rbrace$.
\end{thm}
\begin{thm}
Soit $G$ un groupe algébrique, $H\lhd G$ fermé. \\Alors il existe un $G$-module $(V, \rho)$ de dimension finie tel que $H=\Ker{\rho}$.
\end{thm}
