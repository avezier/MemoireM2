...
\subsection{Généralités}
\begin{defn}[Diviseur premier, Div($X$), diviseur de Weil]
Soit $X$ une variété normale irréductible. Un diviseur premier $D$ est une sous-variété fermée irréductible de codimension 1. On définit Div($X$) le groupe libre engendré par les diviseurs premiers. Un élément de Div($X$) est appelé un diviseur de Weil.
\end{defn}
...
$A_p$ est un anneau local d'idéal maximal $pA_p$. $A_p$ est noetherien et normal car la localisation préserve ces propriétés. De plus $A_p$ est de dimension 1 car on a une correspondance entre les idéaux premiers de $A$ inclus dans $p$, et les idéaux premiers de $A_p$. On conclut que $\Oo_{X,D}$ est un DVR. La valuation associée $\nu_D:k(X)\rightarrow \ZZ$ donne par définition l'ordre d'annulation d'une fonction rationnelle le long de $D$. La propriété ci-dessous montre que les fonctions rationnelles permettent de définir des diviseurs de Weil.

\begin{prop}
Soit $X$ une variété normale et irréductible et $f\in k(X)^*$. Alors $\nu_D(f)=0$ sauf pour un nombre fini de diviseurs premiers $D$.
\end{prop}
\begin{proof}
Soit $f=g/h\in k(X)*$
\end{proof}
