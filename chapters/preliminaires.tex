...
\subsection{Quotients de variétés algébriques sous l'action d'un groupe algébrique}
Soit $G$ un groupe algébrique et $X$ une $G$-variété. En tant que groupe abstrait agissant sur un ensemble, le quotient de $X$ par $G$ est par définition l'ensemble des orbites. Cet ensemble satisfait une propriété universelle, il représente le foncteur $\textrm{Ens}\rightarrow \textrm{Ens}, Y\mapsto \lbrace f\in \textrm{Map}(X, Y)\mid f \textrm{ est constante sur les orbites} \rbrace$. Pour cette raison on $X//G$ est appelé un quotient catégorique. \\ 
On peut ainsi transporter cette définition dans la catégorie des variétés algébriques. Toutefois, il n'est pas clair que ce quotient existe toujours. Dans cette partie on s'intéresse donc à cette question. On remarque que si il existe, il est unique à isomorphisme près, on le nomme le quotient catégorique, et on le note $X//G$.
...
On suppose $X$ affine et $k[X]^G$ de type fini avec $f_1,...,f_r$ des générateurs. Dans ce cadre, l'algèbre des invariants définit une variété algébrique affine $Y$. On définit $\phi :X\rightarrow k^r,\, x\mapsto (f_1(x),...,f_r(x))$. Le comorphisme $\phi^*$ admet une factorisation: $k[t_1,...,t_r]\twoheadrightarrow k[X]^G\xrightarrow{\subset}k[X]$, d'où $Y\simeq \overline{\phi (X)}$. On peut donc voir $X \xrightarrow{\phi} \overline{\phi (X)}$ comme une réalisation du morphisme $\pi: X\rightarrow Y$ associé à $k[X]^G\subset k[X]$. On appelle ce morphisme, le morphisme quotient. De la même manière on voit que tout morphisme $G$-invariant de variétés affines $X\rightarrow Z$ se factorise à travers $Y$. De ce fait, $Y$ semble être un bon candidat pour le quotient catégorique. Toutefois il faut être prudent, par exemple dans \cite{LAGFerrer} 6.4.10, on exhibe un exemple de cette situation qui n'admet pas de quotient catégorique. On a toutefois le résultat suivant:
...
(iii) Soit $Y\subset X$ une sous $G$-variété. Le morphisme induit $Y//G \rightarrow X//G$ est une immersion fermé. On peut ainsi identifier $\pi_Y$ et $\pi_X$ restreint à $Y$. De plus, soit $Y'$ une autre sous $G$-variété, on a $\pi_X(Y\cap Y')=\pi_X(Y)\cap\pi_X(Y')$.\\
(iv) Chaque fibre de $\pi_X$ contient un unique orbite fermé.
\end{thm}
\begin{proof}
(i) Soit $x\in Y$ et $m_x$ l'ideal maximal de $k[X]^G$ correspondant. La fibre $\pi^{-1}(x)$ correspond à l'ensemble des idéaux maximaux contenant l'extension $I$ de $m_x$ dans $k[X]$. Or on a déjà vu que l'extension dans $k[X]$ était injective. ainsi $I$ est un idéal propre qui donc contenu dans un idéal maximal. La fibre étant non-vide, $\pi$ est surjective.\\
(ii) L'existence de la factorisation a déjà était vue. Avec (i) on a maintenant l'unicité.\\
(iii) On note $i$ l'inclusion $Y\subset X$. $\pi_Xi$ est constant sur les orbites de $Y$ d'où l'existence d'un unique morphisme $\phi:Y//G\rightarrow X//G$ tel que $\phi\pi_Y=\pi_Xi$. La projection $k[X] \rightarrow k[Y]$ est un morphisme de $G$-module surjectif. D'après \ref{reynolds}, cette projection induit un morphisme de $k$-algèbre surjectif $k[X]^G \xrightarrow{\phi^*} k[Y]^G$, donc $\phi$ est une immersion fermée.\\
Soit $I$ (resp. $I'$) l'idéal de $Y$ (resp. $Y'$) dans $k[X]$. L'idéal de $Y\cap Y'$ est $I+I'$ et l'idéal de $\pi_X(Y)$ est $\mathcal{I}_{X//G}(\pi_X(Y))=I\cap k[X]^G=R_X(I)$. Ainsi $\mathcal{I}_{X//G}(\pi_X(Y\cap Y'))=R_X(I+I')=R_X(I)+R_X(I')=\mathcal{I}_{X//G}(\pi_X(Y)\cap \pi_X(Y'))$.\\
(iv) d'après (iii), $\pi_X$ envoie deux orbites fermés distincts sur deux points distincts.
\end{proof}

Comme les ouverts affines d'une variété algébrique forme une base de la topologie, on peut rechercher un cadre dans lequel on sera localement dans la situation agréable précédente. C'est la notion de bon quotient.
...
De plus, si les fibres de $\pi$ sont exactement les orbites, on dit que $(Y, \pi)$ est un quotient géométrique.
\end{defn}
