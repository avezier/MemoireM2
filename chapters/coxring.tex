...
On considère comme en \ref{linebundle} l'action de $\GG_m$ sur les fibres de $L$, l'ensemble des points fixes $L_0$, et $L^\times$ son complémentaire, que l'on note également $\widetilde{X}$ par cohérence de notation avec la suite de ce chapitre. En considérant le recouvrement standard $(U_i)_{0\leq i\leq n}$ de $\PP^n_k$ qui trivialise $\Oo_{\PP^n_k}(1)$, on voit que $L^\times$ est constitué de recollements de schémas isomorphes à $\spec_{U_i}( \Oo_{U_i}[t,t^{-1}])\simeq U_i \times_k \GG_m$, ce qui donne $L^\times=\spec_{\PP^n_k}(\oplus_{d\in \ZZ}\,\,\Oo_{\PP^n_k}(d))$. De plus, $L^\times$ est isomorphe à $\AAA_k^{n+1}\setminus\lbrace 0 \rbrace$ par restriction de $\pi$. D'autre part, $p$ se restreint en une application $p^\times:L^\times \rightarrow \PP^n_k$ qui est le quotient géométrique de l'action de $\GG_m$. On résume cela dans le diagramme ci-dessous:
	...
	\begin{center}
	\begin{tikzcd}
  		\overline{X} & \widetilde{X}:=L_X^\times \arrow[l, hook, ""] \arrow[r, hook, "j"] \arrow[d, "p_X^\times"] & L^\times \arrow[d, "p^\times"] \arrow[r, "\simeq"] & \AAA^{n+1}_k\setminus \lbrace 0 \rbrace\\ 
  		&X \arrow[r, hook, "i"] & \PP^n_k &
	\end{tikzcd}\\
	\end{center}
...
Posons maintenant $Y=\overline{X}$ et calculons les sections globales du faisceau d'algèbres divisorielles $\Ss$ sur $Y$ associé à $K=\ZZ D_2$, on note également $\Ss$ sa restriction à $Y^*:=Y\setminus\lbrace0\rbrace=\widetilde{X}$ en se rappelant \ref{isomorphismcodim2CorXreg}. On a $\Ss_{-D_2}(Y)=\mathfrak{p}_2$ d'après \ref{divaff}. Le diviseur $D_2$ est de Cartier sur le cône épointé lisse $Y^*$, ce qui entraîne que le faisceau $\Ss_{-D_2}$ est inversible sur $Y^*$ d'inverse $\Ss_{D_2}$. Ce dernier est la restriction à $Y^*$ du $\Oo_{Y}$-module $(t_4^{-1}\mathfrak{p}_1)^{\widetilde{}}$, comme on le vérifie sur chaque ouvert du recouvrement affine $Y^*=\cup_{i=1}^4Y_{t_i}$.  On a ainsi

\begin{center}
$\Ss_{-D_2}(Y^*)=\mathfrak{p}_2=(t_2, t_4)$, et $\Ss_{D_2}(Y^*)=t_4^{-1}\mathfrak{p}_1=(1, t_4^{-1}t_1)$.
\end{center}
Les applications naturelles $\Ss_{\pm D2}(Y^*)^{\otimes n}\rightarrow \Ss_{\pm nD2}(Y^*)$ avec $n>0$, ne sont pas surjectives a priori. On va calculer localement les sections de $\Ss_{\pm nD2}$ sur les ouverts du recouvrement affine $Y^*=\cup_{i=1}^4Y_{t_i}$, puis les recoller en prenant l'intersection $\Ss_{\pm nD2}(Y^*)=\cap_i \Ss_{\pm nD2}(Y_{t_i})$ dans $k(Y^*)$. D'après \ref{FQCProp2}, on a $\Ss_{\pm nD_2}(Y_{t_i})=\Ss_{\pm D_2}(Y)_{t_i}^n$, et on calcule 
...
On voit que l'on a $\cap_i \Ss_{\pm nD2}(Y_{t_i})\subset \Ss_{\pm D2}(Y^*)^n$ et on en déduit que l'application $\Ss_{\pm D2}(Y^*)^n\xhookrightarrow{} \Ss_{\pm nD2}(Y^*)$ est un isomorphisme. Ainsi $\Ss(Y)$ est engendrée en tant que $\Oo(Y)$-algèbre par les éléments $(z_1,z_2,z_3,z_4):=(1, t_4^{-1}t_1,t_2,t_4)$. De plus d'après les équations suivantes, la composante homogène de degré zéro est incluse dans $k[z_1,z_2,z_3,z_4]$:
...
\end{cons}
\begin{proof}
Les actions de $H$ et $H_X$ sont dues aux graduations respectives de $\Ss$ et $\Rr$ par $K$ et $\clg(X)$. Pour tout ouvert affine $U\subset X$, notons $U_1:=p_X^{-1}(U)$ et $U_2:=p^{-1}(U)$. La projection $\Ss(U)\rightarrow \Rr(U)$ est graduée par $c:K\rightarrow \clg(X)$, et donne une immersion fermée $H_X$-équivariante $U_1\xhookrightarrow{} U_2$ d'image $\Vv_{U_2}(\Ii(U_2))$. Ces immersions se recollent globalement en une immersion fermée $H_X$-équivariante $i$ d'image $\Vv(\Ii)$. 

On a déjà vu que $(\widetilde{X},p)$ est un $H$-fibré principal car c'est localement un $H$-fibré trivial (\ref{lisseGoodQuotient}). Soit $(U_i)_{i\in I}$ un recouvrement ouvert de $X$ tel que $\widetilde{U_i}$ soit un $H$-fibré trivial pour tout $i\in I$. On construit avec cela un morphisme étale et surjectif $\psi:X':=\bigsqcup_{i\in I} U_i\rightarrow X$. On obtient également un morphisme étale surjectif et $H$-équivariant $\phi:H\times X'\rightarrow \widetilde{X}$, où l'action de $H$ se fait sur le facteur de gauche. Puis on trouve un sous-tore $H'$ de $H$ tel que $H_XH'=H$ et $H_X\cap H'$ est fini (\ref{GDiagSubtorusFacteurDirect}). Avec cela on construit d'après \ref{GITEtaleFiniteGroup} un revêtement étale $(H_X\times H'\times X',\pi)$ de $H\times X'$, en faisant le quotient géométrique par l'action du groupe fini $H_X\cap H'$. On en déduit le diagramme commutatif ci-dessous, où le morphisme $\phi$ est étale surjectif $H$-équivariant et le carré cartésien
	
	
	\begin{center}
	\begin{tikzcd}
  		\widehat{X} \arrow[r, hook,"\subset"] \arrow[rd, "p_X"] & \widetilde{X} \arrow[d, "p"] \arrow[rd, phantom,"\square"] & H\times X' \arrow[l, "\phi"] \arrow[d, "p_{X'}"] & H_X\times H'\times X' \arrow[l, "\pi"]\\ 
  		& X  & X' \arrow[l, "\psi"] &
	\end{tikzcd}
	\end{center}

En prenant l'image inverse de $\widehat{X}$ par $\phi\circ\pi$, on obtient une sous-variété $H_X$ stable de $H_X\times H'\times X'$ où l'on rappelle que l'action de $H_X$ se fait sur le facteur de gauche. On en déduit facilement qu'elle est de la forme $H_X\times S$ pour une sous-variété $S$ de $H'\times X'$. On en déduit le diagramme commutatif ci-dessous
	\begin{center}
	\begin{tikzcd}
  		\phi^{-1}(\widehat{X}) \arrow[d, "p_{X'}"]  & H_X\times S \arrow[l, "\pi"] \arrow[d, "p_S"]\\ 
  		X'  & S \arrow[l, ""]
	\end{tikzcd}
	\end{center}
où les deux flèches horizontales correspondent au quotient par l'action du groupe fini $H_X\cap H'$. On en déduit que le carré ci-dessus est nécessairement cartésien et on conclut finalement que l'on a un carré cartésien
	\begin{center}
	\begin{tikzcd}
  		\widehat{X} \arrow[d, "p_X"] \arrow[rd, phantom, "\square"]  & H_X\times S \arrow[l, ""] \arrow[d, "p_S"]\\ 
  		X  & S \arrow[l, ""]
	\end{tikzcd}
	\end{center}
où les flèches horizontales sont étales, surjectives et $H_X$-équivariantes. Cela montre que $(\widehat{X},p_X)$ est un $H_X$-fibré principal car cette propriété se teste par changement de base fidèlement plat (Voir \cite[IV 2.7.1]{EGA} où sont étudiées les propriétés des morphismes permanentes par changement de base fidèlement plat).
...
La variété $\widehat{X}$ est quasi-affine car $\widetilde{X}$ l'est d'après \ref{specrelatifQuasiAff}, et elle est lisse car $X$ est lisse et $(\widehat{X},p_X)$ est un $H_X$-fibré principal (voir \cite[2.3.3]{LinearizationGBrion}). 
