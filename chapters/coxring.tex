...
Soit $U\subset X$ un ouvert, $f\in\Gamma(U,\Ii)$ et $D\in K$. La composante $\clg(X)$-homogène $f_{[D]}\in \Gamma(U,\Ss_{[D]})$ s'écrit de manière unique sous la forme:
...
Puis la remarque \ref{lisseGoodQuotient} nous dit que ce sont en fait des quotients géométriques. Ainsi, le morphisme $\Gamma:H_X\times \widehat{X}\rightarrow \widehat{X}\times_X\widehat{X}$ est surjectif. Comme $H_X$ agit librement, $\Gamma$ est bijectif. On va voir que $\widehat{X}$ est normale, donc $\widehat{X}\times_X \widehat{X}$ également. Ainsi, $\Gamma$ est un isomorphisme. De plus, on va voir que $p_X$ est plat. Comme il est surjectif on obtient que $(\widehat{X}, p_X)$ est un $H_X$-fibré principal. On a déjà vu que $(\widetilde{X},p)$ était un $H$ fibré principal.
...
On remarque avec cette proposition que la factorialité de $R$ entraine sa $K$ factorialité. Munit de ces définitions on va pouvoir généraliser les résultats de divisibilité dans l'anneau de Cox obtenus en \ref{divisibilitePropsFreeCox} dans le cas sans torsion.
