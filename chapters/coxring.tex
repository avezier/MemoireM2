...
\end{cons}
\begin{proof}
Les actions de $H$ et $H_X$ sont dues aux graduations respectives de $\Ss$ et $\Rr$ par $K$ et $\clg(X)$. Pour tout ouvert affine $U\subset X$, notons $U_1:=p_X^{-1}(U)$ et $U_2:=p^{-1}(U)$. La projection $\Ss(U)\rightarrow \Rr(U)$ donne localement une immersion fermée $U_1\xhookrightarrow{} U_2$ d'image $\Vv_{U_2}(\Ii(U_2))$. De plus, ces immersions sont graduées par la projection $c:K\rightarrow \clg(X)$, et se recollent en une immersion $i$ d'image $\Vv(\Ii)$ qui est $H_X$-équivariante. Les mêmes arguments qu'en \ref{specrelatifdivi} nous donne que $p_X$ et $p$ sont des bons quotients car on a
