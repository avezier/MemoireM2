...
\item Tout orbite de dimension minimale est fermée.
\end{enumerate}
\end{prop}
\begin{proof}
...
\item D'après \ref{dimensionfibres}, il existe un ouvert dense de $G.x$ tel que toutes les fibres de cet ouvert ont pour dimension $\dim G-\dim G.x=\dim G_x$.
...
\item Si dim($G.x$) est minimal, alors $\overline{G.x}\setminus G.x$ est vide.
\end{enumerate}
Enfin, si $G$ n'est pas connexe, on écrit $G.x=\cup_{i=1}^{n}g_iG^\circ .x=\cup_{i=1}^{n}G^\circ g_i.x$ car $G^o$ est distingué. Ainsi, $G.x$ est réunion de $G^o$-orbites isomorphes qui sont fermées car il existe au moins une orbite fermée d'après 4) dans le cas connexe. Comme les fermés irréductibles $G^\circ g_i.x$ sont égaux où disjoints, $G.x=\cup_{i=1}^{n}g_iG^\circ .x$ est la décomposition en composantes irréductibles de $G.x$, puis $\overline{G.x}=\cup_{i=1}^{n}\overline{G^\circ g_i.x}$ est celle de $\overline{G.x}$. D'après 1) dans le cas connexe, il existe un ouvert $V_1$ de $\overline{G.x}$ tel que $V_1\cap \overline{G^o.X}=G^o.x$. Soit l'ouvert $V_2=\overline{G.x}\setminus \cup_{i=2}^{n}\overline{g_iG^\circ .x}$, alors $V_1\cap V_2$ est un ouvert de $\overline{G.x}$ inclus dans $G^0.x$. On en déduit que $G.x$ contient un ouvert de $\overline{G.x}$, et on a prouvé 1).

D'après ce qui précède on a $\dim G.x=\dim G^o.x=\dim G^o-\dim (G^o) _x$, or $\dim G^o-\dim (G^o) _x=\dim G -\dim G_x$. En effet, $(G_x)^o \subset(G^o)_x \subset G_x$, d'où $\dim G_x=\dim (G^o) _x$, ce qui finit de prouver 2).

On a $\overline{G.x}\setminus G.x=\cup_{i=1}^{n}\overline{g_iG^o .x}\setminus g_iG^o .x=\cup_{i=1}^{n}g_i(\overline{G^\circ .x}\setminus G^o .x)$ qui est une union finie de fermés de dimension inférieure à $\overline{G .x}$ ce qui prouve 3). On utilise le même argument pour prouver 4) dans le cas général.
\end{proof}

\begin{prop}
Soit $f:G\rightarrow H$ un morphisme de groupes algébriques. Alors l'image topologique $f(G)$ est fermée dans $H$, et $\dim G=\dim\Ker f+\dim f(G)$.
\end{prop}
\begin{proof}
$G$ agit sur $H$ par $g.h=f(g).h$. Les orbites sont les classes à droite de $f(G)$, et elles sont toutes isomorphes. Comme il existe une orbite fermée, on obtient que $f(G)$ est fermé. Pour $h\neq e_H$, le stabilisateur $G_h$ est $\Ker f$, d'où $\dim f(G)=\dim G-\Ker f$.
\end{proof}


\begin{defn}[$G$-module, simple, semi-simple]
Une représentation de $G$, ou $G$-module est un couple $(V, \rho)$ où $V$ est un $k$-espace vectoriel de dimension finie et $\rho$ un morphisme de groupes algébriques de $G$ dans $GL(V)$. On étend cette définition au cas où $V$ est de dimension infinie en demandant que $V$ soit réunion de $G$-modules de dimension finie.
...
\end{thm}
\begin{proof}
D'après la preuve de la proposition précédente, tout élément $f\in\Oo(X)$ est contenu dans un sous $G$-module $V(f)$ de dimension finie. On considère la somme $V$ des sous-espace $V(f_i)_{1\leq i\leq n}$, où les éléments $f_i$ engendrent $\Oo(X)$ comme $k$-algèbre. Ainsi $\Oo(X)$ est engendré comme $k$-algèbre par le sous $G$-module de dimension finie $V$. On introduit le morphisme $\phi:X\rightarrow V^*, x\mapsto ev_x$, c'est une immersion fermée. En effet, il suffit de montrer que $V$ est dans l'image de $\phi^\sharp$. Or $V^{**}\subset \Oo(V^*)$, et pour tout $ev_f\in V^{**}$, on a $\phi^\sharp(ev_f)=ev_f\circ\phi=f$. Enfin, en munissant $V^*$ de la représentation duale, on a $\phi(g.x)(f)=ev_{g.x}(f)=ev_x( g^{-1}.f)=(g^{-1})^t(ev_x)(f)=(g.\phi(x))(f)$, pour tout $g\in G, x\in X,f\in V$. Ainsi $\phi$ est un morphisme de $G$-variétés.
\end{proof}
...
\begin{thm}[Chevalley]
Soit $G$ un groupe algébrique et $H\lhd G$ un sous-groupe distingué fermé. Alors il existe un $G$-module $(V, \rho)$ de dimension finie tel que $H=\Ker{\rho}$.
\end{thm}
\begin{proof}
Soit $V$ et $L$ comme dans le théorème précédent, alors $H$ agit sur $L$ via un caractère $\chi_1$, et on note $V_{\chi_1}:=\lbrace v\in V\mid h.v=\chi_1(h)v,\, \forall h\in H\rbrace$ le sous-espace propre associé à $\chi_1$. On note $V_{\chi_1},...,V_{\chi_n}$ tous les sous-espaces propres de $V$ sous l'action de $H$. Ils sont en somme directe et cette somme est $G$-stable car $H$ est distingué. Comme $L\subset V_{\chi_1}$, on a $H=\lbrace g\in G\mid g.V_{\chi_i}=\chi_i(g)V_{\chi_i}\rbrace$, et on remplace $V$ par $\bigoplus_{i=1}^nV_{\chi_i}$. Maintenant, $G$ agit linéairement sur $\End(V)$ par conjugaison, et même sur $\prod_{i=1}^n\End V_{\chi_i}$ car $G$ permute les sous-espaces $V_{\chi_i}$. Le noyau de cette action est $H$ car pour agir trivialement, un élément $g\in G$ doit stabiliser les sous-espaces $V_{\chi_i}$. Puis sa restriction à chaque $V_{\chi_i}$ doit être dans le centre de $\gln(V_{\chi_i})$. Ainsi $g$ agit par homothétie dans chacun de ces sous-espaces et on conclut $g\in H$. L'autre inclusion est immédiate et on a le résultat.
\end{proof}
...
\begin{proof}
	...
	On peut se ramener au cas précédent en procédant en deux étapes. Dans un premier temps, on quotiente par le sous-groupe distingué connexe $H^\circ$ (on a bien $H^\circ\leq G^\circ$), puis on quotiente par le sous-groupe distingué fini $H/H^\circ$. Il reste donc à prouver le cas $H$ fini, qui sera un corollaire direct du théorème \ref{goodquotientthm}.
\end{proof}

\begin{lem}[Car. zéro]\label{factoInvariantFibers}
Soit $f:X \mapsto Y$ un morphisme dominant de variétés irréductibles, et $g:X \rightarrow Z$ un morphisme constant sur les fibres de $f$.\\
Alors il existe un élément $h\in \Oo(Y)$ non-nul et une factorisation
	\begin{tikzcd}
...
\item Les unités de $k[\GG_m]$ sont les monômes de Laurent. On en déduit que les caractères sont exactement les $t\mapsto t^k, k\in \ZZ$. Par ailleurs, on montrera en \ref{unitGroupFiniteType} que $X^*(G_1\times G_2)\simeq X^*(G_1)\times X^*(G_2)$ pour tous groupes algébriques connexes $G_1$ et $G_2$, d'où $X^*(\dn_n)=\lbrace \textrm{monômes de Laurent à coefficient unitaire} \rbrace \simeq \ZZ^n$.
\end{enumerate}
\end{ex}
...
On remarque que les caractères de $\dn_n$ engendrent $\Oo(\dn_n)$ comme $k$-espace vectoriel, ils en forment donc une $k$-base par le lemme de Dedekind qui assure que les caractères sont libres dans Map$(\dn_n, k)$. On a plus généralement:
...
\begin{proof}
\begin{enumerate}
\item 1)$\implies$2) La restriction $\Oo(\dn_n)\xrightarrow{res_G} \Oo(G)$ est surjective et la restriction d'un caractère est un caractère.
\item 2)$\implies$3) $G$ est abélien, car $\forall \chi\in X^*(G),\,\,g,h\in G$, on a $\chi(gh)=\chi(hg)$. Cette relation est donc vérifiée pour toute fonction régulière sur $G$, on conclut que $gh=hg$. On observe que l'action naturelle de $G$ sur $\Oo(G)$ est semi-simple. En effet, les caractères forment une base de diagonalisation de $\Oo(G)$. Ainsi $G$ est semi-simple par la décomposition de Jordan. Soit $(V,\rho)$ le $G$-module considéré et $W\subset V$ un sous $G$-module de dimension finie, disons $n$. Par la décomposition de Jordan,  $\rho_{|W}(G)$ est semi-simple. De plus, c'est un sous-groupe abélien fermé de $\gln_n$. Il est donc conjugué à un sous-groupe fermé de $\dn_n$. On voit ainsi que $V=\bigoplus_{\chi \in X^*(G)}V_\chi$, où $V_\chi:=\lbrace v\in v\mid g.f=\chi(g)f,\, \forall g\in G \rbrace$. En effet, ils sont en somme directe, et tout élément de $V$ se décompose de cette manière.
\item 3)$\implies$1) On peut supposer $G\subset \gln_n$, et considérer l'action naturelle sur $k^n$ après choix d'une base $(e_1,...,e_n)$. Par hypothèse, on peut écrire $k^n=(f_1)\oplus...\oplus (f_n)$, où les $(f_i)$ sont des sous $G$-modules de dimension $1$. On obtient donc que $G$ est conjugué à un sous-groupe de $\dn_n$.
\end{enumerate}
\end{proof}
...
Montrons que $k[M]$ est de type fini comme $k$-algèbre. Comme $M\simeq \ZZ^r\oplus (\oplus_{i=1}^r \ZZ/d_i\ZZ)$, il suffit de traiter les cas $M=\ZZ$ et $M=\ZZ/d\ZZ$. On a $k[\ZZ]\simeq k[t,t^{-1}]$ qui est de type fini intègre et réduite. D'autre part, $k[\ZZ/d\ZZ]\simeq k[t]/(t^d-1)$ et on voit par le théorème chinois, que cette algèbre de type fini non-intègre est réduite si et seulement si les racine de $t^d-1$ sont simples, ce qui est le cas en caractéristique zéro. 
...
\end{center}
Comme $X^*(G)$ et $X^*(H)$ sont des $\ZZ$-modules libres, $\Ker i^\sharp$ admet un supplémentaire $\ZZ$-libre $V$. On a ainsi $X^*(G)=\Ker i^\sharp\oplus V$, puis $G\simeq \spec k[\Ker i^\sharp] \times \spec k[V]\simeq H'\times H$, où $H':=\spec k[\Ker i^\sharp]$.
...
Les couples $(H, X)$ constitués d'un groupe diagonalisable $H$ et d'une variété affine $X$ munie d'une action de $H$ forment une catégorie dans laquelle les morphismes sont les couples $(\phi,\widetilde{\phi})$, où $\phi:X\rightarrow X'$ est un morphisme de variétés affines, $\widetilde{\phi}:H\rightarrow H'$ est un morphisme de groupes diagonalisables, et pour tout $x\in X, h\in H$, on a $\phi(h.x)=\widetilde{\phi}(h)\phi(x)$.
...
Le petit calcul ci-dessous montre que pour tout $f\in \Oo(X')_{\chi'},\,$ on a $\phi^\sharp(f)\in \Oo(X)_{\widetilde{\phi}^\sharp(\chi')}$. Cela montre que $(\widetilde{\phi}^\sharp,\phi^\sharp)$ est un morphisme d'algèbres graduées car $\widetilde{\phi}^\sharp$ envoie des caractères sur des caractères en tant que comorphisme d'un morphisme de groupes diagonalisables. 
...
Pour l'étude de la géométrie de l'orbite d'un point, les diagrammes ci-dessous seront d'une grande aide. Le diagramme de gauche est obtenu avec \ref{factoInvariantFibers} (ou \ref{groupQuotientGeometrique}) et \ref{ZMTCor}. Le diagramme de droite est obtenu par application du foncteur $\Oo(.)$. Le isomorphismes en pointillés sont uniques faisant commuter ces diagrammes.

	\begin{equation}\label{eq:DiagOrbite1}
	\begin{tikzcd}
		H \arrow[r,"h \mapsto h.x", twoheadrightarrow] \arrow[d,"\pi", twoheadrightarrow] & H.x \\
		H/H_x \arrow[ru, " \phi", swap, dashed]
	\end{tikzcd}
	\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,
	\begin{tikzcd}
		\Oo(H) & & \Oo(H.x) \arrow[lld, "\phi^\sharp", dashed]\arrow[ll, hook, "f_\omega \mapsto f_\omega(x)\chi^{\omega}", swap]  \\
...
Les groupes finis sont linéairement réductifs par le théorème de Maschke. Dans la partie \ref{GroupesDiag} on a vu un autre exemple de groupes linéairement réductifs, les groupes diagonalisables.
\end{ex}
...
\begin{thm}[Hilbert, Nagata]\label{hilbert}
Soit $G$ un groupe linéairement réductif et $X$ une $G$-variété affine. Alors l'algèbre des invariant $\Oo(X)^G$ est de type fini.
\end{thm}
\begin{proof}
Supposons que $X$ soit un $G$-module $V$ de dimension finie. L'action de $\GG_m$ sur $V$ par homothétie donne une $\NN$-graduation $\Oo(V)=\bigoplus_{n=0}^{\infty}\Oo(V)_n$, où $\Oo(V)_n$ est le sous espace des polynômes homogènes de degré $n$. Cette graduation est $G$-stable et se restreint sur l'algèbre des invariants en une $\NN$-graduation $\Oo(V)^G=\bigoplus_{n=0}^{\infty}\Oo(V)^G_n$.  On remarque que $\Oo(V)^G$ est noethérien. En effet, soit $I$ un idéal de $\Oo(V)^G$, et $J$ son extension dans $\Oo(V)$. L'idéal $J$ est un sous $G$-module, donc la contraction de $J$ dans $\Oo(V)^G$ est $R_{\Oo(V)}(J)=IR_{\Oo(V)}(\Oo(V))=I$. On voit donc que la condition de chaîne ascendante est satisfaite sur $\Oo(V)^G$ si elle satisfaite sur $\Oo(V)$, ce qui est le cas car ce dernier est noethérien par le théorème de la base de Hilbert. Cela montre que $\Oo(V)^G$ est de type fini comme $k$-algèbre d'après \ref{GradedPositivNoetherianFG}.
...
Clairement, $Z(S)$ est un sous-groupe distingué de $N(S)$, et le quotient $W=N(S)/Z(S)$ agit sur $S$. La surjection $\Oo(X)\rightarrow \Oo(S)$, induit un morphisme $\Oo(X)^G\xrightarrow{\phi} \Oo(S)^W$. Supposons que l'on ait un ouvert dense $U\subset X$ tel que pour tout $x\in U,\, G.x$ intersecte $S$, alors on voit que $\phi$ est injective. Si de plus, $\Oo(S)^W$ est engendré par des éléments $\phi(f_1),...,\phi(f_r)$, alors $\phi$ est un isomorphisme et $\Oo(X)^G$ est engendré par $f_1,...,f_r$.

\begin{ex}
$G=\gln_n$,\, $X=\mn_n$, $\,g.A=gAg^{-1},\, S=\dn_n,\, U=X_{\disc(\chi )}$, où $\chi:=\det(t\text{I}_n-(x_{ij})_{ij})$ avec $(x_{ij})$ les coordonnées sur $X$. En considérant un élément de $U$, qui a donc ses valeurs propres deux à deux distinctes, on a par un calcul direct $Z(S)=\dn_n$. Puis on a $N(S)=\lbrace\textrm{matrices monomiales}\rbrace$ car la conjugaison préserve les espaces propres. Ainsi, $W$ est isomorphe au groupe symétrique $\Sigma_n$ et agit sur $S$ en permutant les entrées diagonales. On a ainsi $\Oo(S)^W=k[\sigma_1,...,\sigma _n]$, l'algèbre engendrée par les fonctions symétriques élémentaires. C'est une algèbre de polynômes car les $\sigma_i$ sont algébriquement libres. Soient $f_1,...,f_n$ les coefficients du polynôme caractéristique générique. Ce sont des éléments de $\Oo(X)^G$, et on a $f_{i| S}=(-1)^i\sigma _i$, d'où $\Oo(X)^G=k[f_1,...,f_n]$.
\end{ex}
...
\begin{ex}
On considère l'action naturelle de $\gln_n$ sur $\AAA^n$. Le quotient catégorique existe et est un point. En effet soit $f:\AAA^n\rightarrow Z$ constant sur les orbites, alors $f$ est constante car il existe une orbite dense. En revanche il y a une deuxième orbite, c'est le fermé $\lbrace 0\rbrace$. 
\end{ex}
...
\item Chaque fibre de $\pi_X$ contient une unique orbite fermée.
...
\item On note $i$ l'inclusion $Z\subset X$. Alors $\pi_Xi$ est constant sur les orbites de $Z$ d'où l'existence d'un unique morphisme $\phi:Z/G\rightarrow X/G$ tel que $\phi\pi_Z=\pi_Xi$. La projection $\Oo(X) \rightarrow \Oo(Z)$ est un morphisme de $G$-modules surjectif. D'après la proposition \ref{reynolds}, cette projection induit un morphisme de $k$-algèbres surjectif $\Oo(X)^G \xrightarrow{\phi^\sharp} \Oo(Z)^G$, donc $\phi$ est une immersion fermée.
...
\item D'après 3), $\pi_X$ envoie deux orbites fermées distinctes sur deux points distincts.
...
Soit $G$ un groupe linéairement réductif et $X$ une $G$-variété. Un couple $(Y, \pi)$ où $Y$ est une variété et $\pi$ un morphisme $X\rightarrow Y$ est un bon quotient s'il vérifie:
\begin{enumerate}
...
\end{prop}
\begin{proof}
Le bon quotient existe d'après \ref{ExGpLinReductive} et \ref{goodquotientthm}, il s'agit du comorphisme de l'inclusion $\Oo(X)^G\subset \Oo(X)$. De plus comme les orbites sont fermées il s'agit d'un quotient géométrique toujours d'après \ref{goodquotientthm}. Soit $f\in\Oo(X)$, on considère le polynôme $\prod_{g\in G}(t-g.f)$ en la variable $t$. On constate que c'est un polynôme unitaire de $\Oo(X)^G[t]$ dont $f$ est une racine. Cela prouve que $\Oo(X)$ est entier sur $\Oo(X)^G$, et donc une extension finie. Le morphisme $\pi$ est donc fini. 
\end{proof}

\begin{prop}\label{GITEtaleFiniteGroup}
Soit $X$ une $G$-variété avec $G$ fini agissant librement et telles que les orbites admettent des voisinages ouverts affines. Alors le bon quotient de $X$ par $G$ est un quotient géométrique. De plus, $(X,\pi)$ est un revêtement étale de $X/G$.
\end{prop}
\begin{proof}
Voir \cite[II.7]{MumfordAbelianVarieties}
\end{proof}

\subsection{Un exemple: La construction du Proj}
\label{ProjConstruction}
Dans cette partie, on va détailler une construction qui à la fois éclaire et généralise la construction de la variété algébrique $\PP^n_k$. On considère $A$ une algèbre affine $\NN$-graduée et on pose $X:=\spec(A)$ ainsi muni d'une action de $\GG_m$. Une orbite $\GG_m.x$ est de dimension 0 ou 1. Si elle est de dimension 0, c'est un point fixe car $\GG_m$ est connexe. Si elle est de dimension 1, elle est soit fermée, soit son adhérence est constituée de $\GG_m.x$  et d'une réunion de points fixes, en fait un seul comme on va le voir.
...
On remarque que $A^{\GG_m}=A_0=A/A_{>0}$, et comme le bon quotient $Y_0:=X//\GG_m=\spec A_0$ paramètre les orbites fermées, on obtient en particulier que si l'orbite $\GG_m.x$ n'est pas fermée, son adhérence contient un unique point fixe. En résumé, $W:=X\setminus F$ est la réunion des orbites de dimension $1$, et elles sont fermées dans $W$. On va maintenant montrer que $W$ admet un quotient géométrique, c'est la construction du $\proj$ de $A$.
