...
\item Tout sous groupe fermé de $\gln_n(k)$ est un groupe algébrique. Par exemple D$_n$, le sous groupe des matrices diagonales est défini par l'annulation des coefficients hors-diagonale.
\end{enumerate}
\end{ex}
...
Le stabilisateur d'un point $x$ de $G$ est un sous-groupe fermé de $G$. En effet, c'est la fibre de $x$ pour l'application d'orbite $a_x$. 
...
Montrons que l'action est algébrique. Comme les translatés de $f$ engendrent $V(f)$, il suffit de montrer que pour tout $l\in V(f)^*$, $h,g\in G$, on a $g\mapsto l(g.(h.f))\in \Oo(G)$. On prolonge $l$ en $l'\in \vect_k(\psi_1,...,\psi_r)^*$. On a $ g\mapsto l(g.(h.f))=\sum_{i=1}^r\phi_i((gh)^{-1})l'(\psi_i)\in \Oo(G)$. 
Finalement $\Oo(X)=\cup_{f\in \Oo(X)}V(f)$ est un G-module.
\end{proof}
...
\end{thm}
\begin{proof}
D'après la proposition précédente on voit que $\Oo(X)=k[f_1,...,f_n]$ est engendré comme $k$-algèbre par un sous $G$-module $V$ de $\Oo(X)$ de dimension finie, en prenant par exemple la somme des $V(f_i)$ pour $1\leq i\leq n$. On a une immersion fermée $\phi:X\rightarrow V^*, x\mapsto ev_x$. Pour montrer cela, il suffit de voir que $V$ est dans l'image de $\phi^\sharp$. En effet, on a $V^{**}\subset \Oo(V^*)$, et pour tout $ev_f\in V^{**}$, on a $\phi^\sharp(ev_f)=ev_f\circ\phi=f$. Enfin, en munissant $V^*$ de la représentation duale, on a pour tout $g\in G, x\in X,f\in V$, $\phi(g.x)(f)=ev_{g.x}(f)=ev_x\circ g^{-1}.f=(g^{-1})^t(ev_x)(f)=(g.\phi(x))(f)$, ainsi $\phi$ est un morphisme de $G$-variétés.
\end{proof}
...
\begin{thm}[Chevalley]
Soit $G$ un groupe algébrique et $H\leq G$ fermé. \\Alors il existe un $G$-module $V$ de dimension finie et une droite $L\subset V$ telle que $H=Stab_G(L):=\lbrace g\in G\mid g.v\in L,\forall v\in L\rbrace$.
\end{thm}
...
\item $\Oo(G)=\textrm{Vect}_k(X^*(G))$.
\item Tout $G$-module est somme directe $G$-modules de dimension 1.
\end{enumerate}
\end{prop}
\begin{proof}
\begin{enumerate}
\item 1)$\implies$2) La restriction $k[D_n]\xrightarrow{res_G} \Oo(G)$ est surjective et la restriction d'un caractère est un caractère.
...
Un groupe diagonalisable est un groupe algébrique $G$ tel que $\Oo(G)=\textrm{Vect}_k(X^*(G))$. Un tore est un groupe diagonalisable connexe.
\end{defn}
...
ev$_G$ est surjective: Soit $\phi\in\chi^{**}(G)$. On a un prolongement unique de $\phi$ en un morphisme de $k$-algèbre $k[X^*(G)]=\Oo(G)\rightarrow k$ qui est donc de la forme $\Oo(G)\rightarrow k, f\mapsto f(g)$ pour un $g\in G$. En restreignant à $X^*(G)$, on trouve que $\phi=\textrm{ev}_G(g)$.
\end{proof}
...
Alors $k[M]$ est de type fini, réduite et on a $k[M_1\oplus M_2]\simeq k[M_1]\otimes k[M_2]$. De plus, $k[M]$ est naturellement muni d'une structure d'algèbre de Hopf et on a $\Oo(G)=k[X^*(G)]$ et donc $G=\spec\circ k[X^*(G)]$.
\end{prop}
\begin{proof}
...
\begin{cons}
Soit $H$ un groupe diagonalisable et X une $H$-variété affine. Le $H$-module $\Oo(X)$ est somme directe de $H$-module de dimension 1 d'après la partie précédente et on peut écrire: 
\begin{center}
$\Oo(X)=\bigoplus_{\chi \in X^*(H)}V_\chi$,  où $V_\chi:=\lbrace f\in \Oo(X)\mid h.f=\chi(h)f,\, \forall h\in H \rbrace$ 
\end{center}
...
\begin{multicols}{2}
	...
	\begin{center}
		\begin{tikzcd}
  		k[H]\otimes \Oo(X)   & \Oo(X) \arrow[l, "a_1^*"] \\ 
